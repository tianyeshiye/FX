...
public abstract class MacdPNPointParent {
	...
	public List<String> ea(List<MacdDataBean> beanList, List<MacdResultBean> resulList, List<String> rateList) {
		...
		print(beanList, resulList, logList, rateList);

		return logList;
	}

	private List<String> print(List<MacdDataBean> beanList, List<MacdResultBean> resulList, List<String> logList, List<String> rateList) {
...
//		System.out.println("真实盈亏比:" + winLoseRate);
		...
		output.add(maxIntervalStopValue);
		output.add(Float.valueOf(winChangesSum));
		output.add(Float.valueOf(loseChangesSum));
		output.add(getRate(winChangesSum, loseChangesSum));

		String outputString = Joiner.on(",").join(output);
		
		rateList.add(outputString);
